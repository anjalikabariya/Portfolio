...
export default function Hippo() {
  ...
  const { selectedElement : { position: { angle: angleLeftEye } } }= useMightyMouse(true, "left-eye", { x: 40, y: 40 });
  const {selectedElement : { position: { angle: angleRightEye } } } = useMightyMouse(true, "right-eye", { x: 40, y: 40 });
